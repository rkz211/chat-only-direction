...
**Version:** v1.4-draft, 2026-07-12 — v1.0 body accepted by the director; Appendix C (maintenance) added 07-03; Appendix D (the finishing phase) added 07-11. v1.3 (07-12) folds in the **independent black-box tester** (new §5.1) and its consequences through §6.2, §8, and Appendix D, and replaces Appendix D's placeholder convergence figure with the first measured per-game defect-rate data (≈2→1 bugs/game, 07-11→07-12). v1.4 (same day) adds the graduated verification-fidelity ladder — dev debug passes → independent API-level passes → full human-UI real-browser testing, climbed as the defect rate calms (§5.1, Appendix D). The convergence curve is finalized when the artifact ships bug-free.
