...
**Version:** v1.2-draft, 2026-07-11 — v1.0 body accepted by the director; Appendix C (maintenance session) added 07-03; Appendix D (the finishing phase / autonomous debug-polish loop) added 07-11 as a PRE-RELEASE DRAFT with a placeholder convergence figure, to be finalized when the artifact ships bug-free
...
> **Status of this appendix.** This is a *pre-release draft*, written 2026-07-11 while the phase it describes is still running. Its argument is complete; its central quantitative claim — the convergence curve — is deliberately left as a marked placeholder to be filled from the deploy log at release, so that the number is evidence rather than assertion, consistent with the rest of this report. It will be rewritten once the artifact ships bug-free.
...
**The loop's shape.** Two agents run continuously against one repository. A **tester** plays the product genuinely — deciding each turn from the screen, across a rotation of characters and playstyles — and files defects to a shared queue. A **dev** (the same triage-fix-gate-deploy cycle the body already describes, now on a short timer) drains the queue: reproduce, fix, corpus-lock, hot-deploy, and answer back. The two agents never speak directly; a **queue site** is their entire communication medium — the tester's reports and the dev's flip-notes and a curated release-notes channel are the whole protocol. Over one representative week the loop absorbed several hundred reports across [N] hot deploys while the corpus grew from [X] to [Y] locked scenarios.
...
**The evidence — instrumented, and an early signal [final curve to be inserted at release].** The claim that the loop converges is checkable from the record, and as of this draft it is directly instrumented rather than inferred: the queue site now captures a **per-game defect rate** — reports filed per playthrough — which is the correct normalization, since it controls for how much the tester played rather than merely counting raw reports over calendar time. This is the figure that turns the appendix from argument into evidence. **The preliminary reading is already downward** — the per-game defect rate is trending toward zero under the frozen design, exactly as the convergence-by-construction argument predicts — but this draft deliberately does *not* freeze the number, because a convergence claim is only honest once the artifact has actually reached release and the curve is complete. `[FIGURE D-1: per-game defect rate from first frozen-design commit to release, early trend downward, to be finalized AT RELEASE.]` If the curve reaches zero, the finishing phase is a demonstrated convergence process and the method has a full documented lifecycle. If it plateaus above zero, that is the honest and equally-important finding: a floor of irreducible LLM-adjudication defects the corrective guard stack cannot close, and the empirical trigger for the deferred architectural move (replacing the LLM adjudicator with a deterministic rules engine for the offending verbs). Either outcome is reported; the early downward trend is noted here as suggestive, not conclusive.
...
## Appendix E: Working Notes — the Independent Black-Box Tester (DRAFT, to fold into §5 / §6.2 / §8 / Appendix D)

> **Status.** Unincorporated working notes, captured 2026-07-12 during a director review session. Not yet integrated into the body; recorded here so the resolution of §6.2's stated limit is not lost, and to be folded into §5 (verification), §6.2 (the self-verification blind spot), §8 (limitations), and Appendix D (the finishing loop) at the next revision. Consistent with the rest of this report, the claim is stated first and the honesty envelope adjusted explicitly.

**The claim to fold in.** The report's one structurally unfixed limit — the correlated act-and-check error of §6.2, restated in §8 as *"a second independent verifier is the missing control"* — is, in current practice, addressed by construction. The control is not missing.

**The mechanism — an independent, black-box, UI-only tester.** The agent that builds is never the agent that tests (beyond the first draft or two). Verification is performed by a **separate, unconnected agent** under three constraints that together make it a faithful stand-in for the customer:

1. **No implementation access** — no source, no git history, no backend, no game-state/DB. It cannot rationalize from the implementation, so it cannot inherit the builder's blind spot. (A tester that could read the source or the state would begin reasoning like the builder and reintroduce the very correlation the independence exists to break.)
2. **Forced through the actual human UI** — no backdoor API, no shortcut into the engine.
3. **Real browser automation, not faked** — genuine end-to-end against the deployed front end, so every interaction it reports is one a real human could actually have, and every defect it finds is a real user-facing defect.

**Why the black-box constraint costs no diagnostic power — the detect → reconstruct → diagnose handoff.** The obvious objection is that a state-blind tester surrenders the report's most productive instrument (§5's genuine play, which reads ground-truth state JSON precisely because narration hides state bugs). It does not, because *detection* and *diagnosis* are split across the two agents:

- The **independent black-box tester DETECTS** the failure from the pure user surface — the correlation is broken exactly here, where it matters.
- The **dev then RECONSTRUCTS the tester's exact state and DIAGNOSES** with full ground truth. The dev holds deep stored knowledge and logs of each game and its state, and the system is deterministic and logged (pre-rolled dice, state JSON, seeds), so the tester's scenario replays exactly — the dev can "be its own eyes there" *after* the independent flag, without having biased the detection.

The result is **independence at detection and full-state visibility at diagnosis, with neither contaminating the other.** This dissolves the tradeoff between independence (which argues for no state access) and diagnostic power (which argues for state access): the method obtains both, in sequence, by construction.

**How this revises the honesty envelope.** §6.2's *"the base rate of uncaught correlated errors is unknown"* and §8's *"a second independent verifier is the missing control"* should be narrowed. For any defect that is **user-observable**, the independent black-box tester *is* that second verifier, and its state-blindness is repaired by dev reconstruction. The residual limit shrinks to defects that are *neither user-observable nor state-reconstructable* — a strictly smaller class than v1.2 concedes. Appendix D's finishing loop should be re-described accordingly: the tester and dev are not merely "two agents against one repo" (shared context, the weaker claim) but an **asymmetric pair** — a *state-blind independent detector* and a *full-state reconstructing diagnostician* — and that asymmetry is the load-bearing design, not an incidental division of labor.

**One-line summary for the abstract/intro, if it survives review:** *verification without the correlated-blind-spot tax* — independence is enforced by access-denial (no source/state) and by fidelity (real UI, real browser), while diagnostic depth is preserved by deterministic-replay reconstruction on the dev side.
