...
**Version:** v1.5-draft, 2026-07-12 — v1.0 body accepted by the director; Appendix C (maintenance) added 07-03; Appendix D (the finishing phase) added 07-11. v1.3 (07-12) folds in the **independent black-box tester** (new §5.1) and its consequences through §6.2, §8, and Appendix D, and replaces Appendix D's placeholder convergence figure with the first measured per-game defect-rate data (≈2→1 bugs/game, 07-11→07-12). v1.4 (same day) adds the graduated verification-fidelity ladder — dev debug passes → independent API-level passes → full human-UI real-browser testing, climbed as the defect rate calms (§5.1, Appendix D). v1.5 (same day) generalizes the convergence metric from "per-game" to a domain-general **per-run defect rate** — bugs per full end-to-end feature use (a game for a game engine, a completed sales loop for a sales feature). The convergence curve is finalized when the artifact ships bug-free. v1.6 (same day) corrects an external size claim: the line-count figures in this report are the **2026-07-01 reporting snapshot** (engine 14.2k, tests 8k) and are deliberately held there to stay consistent with the 334-commit / 84-hour window; development has continued and, as of 2026-07-12, the engine is ~29k lines and the test apparatus ~20.8k (~51k lines of Python total) — see Appendix D.
...
### 5.1 Independent verification: the black-box tester

The apparatus above has a known structural weakness: the same agent writes the system, writes the test, and interprets the result, so a blind spot can sit on both sides of the verification boundary at once (§6.2 is the observed instance). The control this class demands is a *second, independent* verifier, and the method now supplies one — **the agent that builds is never the agent that tests** (beyond the first draft or two). Verification is performed by a separate, unconnected agent under three constraints that together make it a faithful stand-in for the customer:

1. **No implementation access** — no source, no git history, no backend, no game-state or database. It cannot rationalize from the implementation, so it cannot inherit the builder's blind spot. (A tester that could read the source or the state would begin reasoning like the builder and reintroduce the very correlation the independence exists to break.)
2. **Forced through the actual human UI** — no backdoor API, no shortcut into the engine.
3. **Real browser automation, not faked** — genuine end-to-end against the deployed front end, so every interaction it reports is one a real human could have had, and every defect it finds is user-facing.

**Fidelity is climbed, not paid up front.** These three constraints describe the tester in its strongest form — an independent agent driving the real UI in a real browser — but that form is also the slowest to run at volume, and the method reserves it for where it pays. Verification climbs a fidelity ladder matched to the falling defect rate: the dev first runs a few of its own debug passes; the independent testers then run **API-level bug passes** (still independent — no source, no state — but hitting the interface below the browser, which is far faster to drive at volume); and only as bugs calm does the loop **graduate to full human-UI, real-browser testing**, the faithful-but-expensive tier. The economics are the product's own model cost ladder (§2, Appendix B — climb to the lowest rung that passes) applied to verification: spend the cheap, fast methods while they are still yielding bugs, and escalate to the costly high-fidelity method only for the tail they can no longer reach — the user-observable, correlated-blind-spot class that genuine UI play surfaces and nothing cheaper does. The "every defect is user-facing" guarantee holds strictly at the top rung; the lower rungs knowingly trade a slice of fidelity for the speed to clear the early bulk.

The obvious objection is that a state-blind tester surrenders the report's most productive instrument — genuine play with ground-truth inspection (instrument 5 above), which reads state JSON precisely because narration hides state bugs. It does not, because *detection* and *diagnosis* are split across the two agents. The **independent black-box tester detects** the failure from the pure user surface — the correlation is broken exactly here, where it matters — and the **dev then reconstructs the tester's exact state and diagnoses** with full ground truth: the dev holds deep logs of each game and its state, and the system is deterministic and logged (pre-rolled dice, state JSON, seeds), so the tester's scenario replays exactly, and the dev becomes "its own eyes there" *after* the independent flag, without having biased the detection. The method thereby obtains **independence at detection and full-state visibility at diagnosis, with neither contaminating the other** — verification without the correlated-blind-spot tax.

§6.2 documents the blind-spot episode this control answers — the case, on the primary artifact, before the independent tester was in place.
...
The episode also hardened into a standing method for a problem specific to this domain (we do not claim it generalizes beyond software with a balance requirement). Class balance is not a player-independent property — there is no fact of "is the Wizard balanced" in the abstract, only *balanced relative to a player of some skill*; a purely mathematical symmetry approach implicitly assumes a null player (equalize raw numbers), which is precisely why it mis-scores asymmetric "thinking" classes that are meant to be numerically behind and made even by skilled play. The lever is that the test agent, over many sessions, *becomes* a genuinely skilled player and can then pilot every class at matched competence and diligence — which holds the confounding variable, operator skill, constant across classes. Any outcome gap observed under that condition is a property of the design rather than an artifact of one class being played better. This is a second capability of an agent test fleet, distinct from throughput (many tireless runs): *uniformity* — one consistent competence replicated across all variants, a controlled condition a human QA team cannot cheaply hold (differing aptitudes, learning effects). It does not replace a separate deterministic balance simulator (a stat-model reusing the engine's own math over hundreds of RNG-free trials), which measures the mechanical floor — whether a party's raw numbers can win; it measures the skill-expression ceiling that simulator structurally cannot reach, because a stat-sim does not kite, control, or exploit terrain. The two are the balance analogue of this report's two-mode verification: deterministic sim for the numbers, skilled genuine play for what only skilled play reveals.

The same maturation closes a loop worth naming. The skilled test agent produces three artifacts from one competence: it *defines* the reference skill by playing at it, *measures* balance against that reference, and — because it has genuinely learned each class — *authors the player-facing strategy guides*. Sharing a single source, the three are consistent by construction: the game is tuned for the skill the guides teach, and the guides teach the skill the game is tuned for. This resolves an apparent tension — balance calibrated to a skilled reference player, yet meant to reflect a real player's initial knowledge — because the guide *raises* initial knowledge toward the calibration point, making the reference-player assumption self-fulfilling rather than aspirational. A design of skill-expressive classes is therefore safe rather than a novice trap only because the oracle that verifies "balanced when played well" is the identical entity qualified to write the manual for playing well. The boundaries are the honest ones: the equal-skill baseline reflects a reference player, not where real players eventually take the game (emergent combinations and degenerate metas beyond the agent's ceiling still require separate balancing), and the claim is comparative — better in this domain than pure-symmetry math — not a general result.
...
**Post-report growth (2026-07-12).** Development continued after the reporting window — notably a 2026-07-10/11 rules-adjudication hardening campaign. As of 2026-07-12 the engine is ~29,000 lines (142 modules) and the test apparatus ~20,800 (≈51,000 lines of Python in total). Every line-count figure elsewhere in this report is deliberately held at the 2026-07-01 snapshot so that it stays consistent with the 334-commit / 84-hour window measured here; this note is the single pointer to the current size.
...
1. **n = 1 — but on the generalization axis only.** One primary project, one director, one agent. Nothing here is a controlled measurement; there is no baseline of the same developer working conventionally. The second artifact (§6.7) shows the mechanisms transferring to a different domain, but it shares the director and agent, so it is domain transfer within the dyad, not independent replication. *One clarification the denominator deserves (post-report note; snapshot figures §2/§7 held as-is):* "n = 1" correctly sizes the **generalization** question — one architecture, one domain, one model generation — and no quantity of hours widens it. It does **not** size the **recurring-phenomenon** observations. The mechanisms this report leans on (the correction-treadmill, the one-sentence delta unstick, distributional steering) were each seen across the project's full arc — by the 2026-07-23 log, **1,610 commits over 38 distinct active working days** (≈460–680 attended direction-hours by the director's 12–18 h/day loop), on the order of a few thousand direction decisions — so as observations of *those patterns* they are not single-instance. The right denominator tracks the claim: generalization claims are n = 1; per-decision pattern claims are n ≈ thousands.
...
4. **The verification asymmetry is real, and now partly controlled.** §6.2 is one observed instance of correlated act-and-check error. On the primary artifact the base rate of *uncaught* instances was unknown and unknowable from inside a single-agent loop. Since then the independent black-box tester (§5.1) supplies the second verifier §6.2 named as missing: for **user-observable** defects the act and the check no longer share a mind, and the tester's state-blindness is repaired by dev-side deterministic-replay reconstruction. The residual — now a strictly smaller class — is defects that are *neither* user-observable *nor* state-reconstructable; for those the asymmetry remains.
...
- Structural rot audit: periodically read the wiring layers against their own contracts — behavioral gates see what the system does, never what it is becoming. (Ratified during the session Appendix C describes.)

## Appendix C: One Maintenance Session, End to End

The body of this report describes construction. A fair objection: greenfield building is the easy case — does the method survive *maintenance*, where most software cost actually lives? This appendix documents one complete maintenance cycle that occurred the day after the v1.0 body was accepted, reported here exactly as the record shows it. Director attention for the entire cycle: roughly six short chat messages.

**1. The audit (a new instrument, ratified the same day).** The director asked the agent to audit the codebase it had built — the first *structural* review, as opposed to the behavioral gates that had run throughout construction. The audit found what behavioral gates cannot: the orchestrator (contractually "pure wiring — must never contain game logic") had accreted to 733 lines with 35 broad exception handlers, patch by individually-reasonable patch; fourteen exception swallows across the engine logged nothing; a load-bearing thread-safety assumption existed nowhere in writing; plus a permissive CORS posture, error-detail leakage, and a deprecated naming shim. Each finding traced to a mechanism, not a style preference. The director dispositioned all six in a single message — *"1) yes… 2) yes, 3) yes, 4) ignore, 5) do, 6) fix — and add a rot-audit rule"* — and the new rule ("behavioral gates see what the system does, never what it is becoming") entered the process corpus that hour.

**2. Coordination across parallel sessions.** The work was explicitly held — the director's call — until a *second* agent session (mid-playthrough on the same repository, §7's multiplexing in action) finished and committed. The executing session then re-scanned the fresh tree before touching it: the audit had been performed against a pre-commit snapshot, and the other session's night of work might have changed any finding.

**3. Execution behind the gate.** The orchestrator was reduced 733 → 569 lines by extracting each embedded concern to a named single-concern module (per-tick hygiene, free actions, L1 delta guards, narration context); every silent swallow gained a log line; the thread-safety invariant was written into the two module contracts it governs plus a newly bootstrapped in-repo lessons ledger; the security posture was tightened (origin allowlist, sanitized 500s with reference ids); the shim was deleted. All of it behind the standard gate.

**4. The gate caught a cross-session miss.** The first regression run came back 568/569 — and the red case was **pre-existing**: verified by stashing all changes and re-running on the untouched tree. The *other* session's redesign of a game mechanic (a two-tier ward system) had deliberately changed semantics, added locks for the new design, and missed updating one old lock that still asserted the previous behavior. The executing session repaired the stale lock to the ratified design and added the complementary case. One session's gate discipline had audited another session's work — verification working *across* the multiplexed workflow, not just within one loop.

**5. The deploy smoke caught a production-facing bug nobody had ever seen.** After deploying to the live serverless backend, the standard smoke test (create campaign → play a turn → delete) returned one anomalous field: `freeAction: false` on a plain "look around." Pulling the thread revealed that a QA instrument — a raw structural debug dump of the current area — was intercepting every real player's *most common action*, ungated, and had been for weeks. It was invisible precisely because its output looked intentional: informative text, no error, every gate green. The director's entire directive: *"that's an error to fix, go ahead."* The dump was gated to QA mode (exactly as its sibling teleport command always had been), three regression locks were added covering all three behaviors, and the fix was live-verified on production within the hour — players now receive narrated prose for "look around," apparently for the first time.

**6. The refactor validated itself within 24 hours.** The next day's tree shows a *different* session's findings — two new narration/state-desync guards — landing inside the modules this session had just extracted, in exactly the right single-concern homes, with the orchestrator untouched. The structure absorbed new concerns instead of re-accreting them, which is the only test of a refactor that matters.

Maintenance mode, then, runs on the same mechanisms as construction — self-triaged reports, one-sentence direction, gates, corpus law — plus two that only appear after there is something to maintain: **structural auditing** (drift is invisible to behavioral gates; someone must periodically read the architecture against its own contracts) and **cross-session verification** (in a multiplexed workflow, each session's gates check the others' work). Both are now corpus rules. The failure modes were also the familiar ones: everything that went wrong — the accretion, the stale lock, the ungated debug path — was a small, individually-invisible drift that only an instrument, never inspection-by-vibes, was going to catch.

---

## Appendix D: The Third Act — the finishing phase (DRAFT, to be finalized at release)

> **Status of this appendix.** A *pre-release draft*: the phase it describes is still running. Its argument is complete; as of v1.3 (2026-07-12) its central quantitative claim — the convergence curve — carries its first two measured points (≈2→1 bugs/game, 07-11→07-12) rather than a bare placeholder, but the curve is finalized only when the artifact ships bug-free. Rewritten at release.

The body of this report describes construction (§3–§6). Appendix C describes maintenance. Both still have a human directing turn by turn. This appendix documents the phase that comes *after* the design is fixed and *before* release — the debug-and-polish phase — and corrects a framing error the author initially made about it.

**It is not a new paradigm; it is the method's third act.** Chat-only direction has a lifecycle on a single artifact: (1) **build** by direction — the human directs turn by turn; (2) **polish** by an autonomous dev↔test loop — the subject of this appendix; (3) **release** at zero bugs. The phases are sequential and non-interchangeable: you cannot *start* a project in phase two. There is nothing yet to test, no design to hold fixed, no corpus to gate against. You start by directing the build, exactly as the body describes, and you *arrive* at the loop. It is where directed construction goes to be finished, not an alternative to it.

**The loop's shape — an asymmetric pair.** Two agents run continuously, and their asymmetry is the design, not an accident (§5.1). A **tester** — an *independent* agent with no source, git, backend, or state access, driven through the real UI by real browser automation — plays the product genuinely, deciding each turn from the screen across a rotation of characters and playstyles, and files defects to a shared queue. A **dev** drains the queue on a short timer: it **reproduces by reconstructing the tester's exact state** from logs and deterministic replay (the detect→reconstruct→diagnose handoff of §5.1), fixes, corpus-locks, hot-deploys, and answers back. The two agents never speak directly and do not share context; a **queue site** is their entire communication medium — the tester's reports, the dev's flip-notes, and a curated release-notes channel are the whole protocol. This asymmetry is what makes the loop a genuine independent-verification instrument rather than one agent grading its own homework twice. Over one representative week the loop absorbed several hundred reports across [N] hot deploys while the corpus grew from [X] to [Y] locked scenarios. The loop also climbs the verification-fidelity ladder of §5.1 as it converges: the high-defect-rate early stretches are cleared with fast dev debug passes and independent API-level passes, and full human-UI real-browser testing — the faithful but slow tier — is brought in as the per-game rate calms, so the most expensive method is spent only on the tail the cheaper ones can no longer reach.

**Why this is still direction, not autonomy.** The distinction is the load-bearing one, and it is easy to get wrong (the author did). The loop's autonomy is *scoped to bug-fixing*. Absent human direction, the agents do not invent features or redesign systems — they drive the defect count toward zero, and only that. Every feature and balance decision still routes through the director exactly as in phase one: the dev agent *asks* ("should the Wight resist nonmagical weapons?"), and the human decides, ignores, modifies, or occasionally hand-injects a feature. The visible burst of "feature development" during this phase was, on inspection, the human answering balance questions and choosing to add a few things — not the loop acting on its own initiative. What changed between phase one and phase three is not *whether* the human directs but *how often*: once the design is fixed, there is simply less to decide, so direction thins from turn-by-turn conversation to occasional policy, and the medium shifts from chat to the queue. The title of this report still holds. It is the same directing, at lower frequency, through a different channel.

**The terminal condition — and why the loop converges by construction.** An earlier draft of this report worried that an always-running loop has "no definition of done" and might run forever patching an LLM that keeps finding new ways to lie. That was the wrong worry, for a structural reason. When the human stops asking for changes, the *only* work the loop can generate is bug-fixing — and bug-fixing has a floor that feature development does not. Each fix removes a defect and locks it out of the corpus permanently; no new fix can reintroduce a locked defect without turning the gate red. So the reachable-defect set is monotonically shrinking under a fixed design. The terminal condition is therefore real and simple: **zero open defects = done = release.** The loop is not open-ended; it is a convergence process with an explicit stopping rule, and "polish to zero, then ship" is the whole of it.

**The evidence — instrumented, and an early measured signal.** The claim that the loop converges is checkable from the record, and it is directly instrumented rather than inferred: the queue site captures a **per-run defect rate** — bugs found per full end-to-end use of the product — the correct normalization, since it controls for how much the tester exercised the system rather than merely counting raw reports over calendar time. The unit is domain-general: a *run* is one complete end-to-end feature use — a played game for a game engine, a completed sales loop for a sales feature, an order from intake to fulfillment for an ops system. Here a run is a playthrough, so we report it as bugs/game. This is the figure that turns the appendix from argument into evidence, and the first two frozen-design readings are in and downward: the per-game defect rate fell from **≈2.0 bugs/game (2026-07-11) to ≈1.0 bugs/game (2026-07-12)** — a roughly day-over-day halving under the frozen design, exactly the shape the convergence-by-construction argument predicts. The number is *not yet frozen*: a convergence claim is only complete once the artifact reaches release and the curve bottoms out, so the two points are reported as a measured early trend, not a finished result. `[FIGURE D-1: per-run defect rate by day (here per-game — one run = one playthrough), first frozen-design commit → release. Measured so far: ~2.0 (07-11) → ~1.0 (07-12), trending down; finalized AT RELEASE.]` If the curve reaches zero, the finishing phase is a demonstrated convergence process and the method has a full documented lifecycle. If it plateaus above zero, that is the honest and equally-important finding: a floor of irreducible LLM-adjudication defects the corrective guard stack cannot close, and the empirical trigger for the deferred architectural move (replacing the LLM adjudicator with a deterministic rules engine for the offending verbs). Either outcome is reported; the early downward trend is noted here as suggestive, not conclusive.

**Honest envelope.** The autonomy here is *supervised*, not unbounded, and the limits must be stated. The loop runs inside a human-set policy frame (which reports are real bugs versus player-skill complaints; which balance changes ship) and depends on a running session to host its timer — it is not a daemon that survives indefinitely unattended. Its correctness rests entirely on the same behavioral instruments as the rest of the method (the corpus, genuine play, deploy smokes); a defect no instrument can see is a defect the loop cannot fix, and the guard stack's growth is itself a watch item — a corrective layer that only ever grows is a signal to change architecture, not to keep patching. And "zero bugs" is zero *observed* bugs against the current test surface, not a proof of correctness. These are the same limits §8 states for the method as a whole, inherited by its final phase.

**What the third act adds to the thesis.** It supplies the definition of done the body left open (bug-free = release), and it shows the method's economics *improving* across the lifecycle: human attention per unit of progress falls toward the end not because the human withdraws but because a fixed design leaves less to decide. Construction relocated expertise from typing to directing; the finishing phase relocates it again, from directing to governing — the human sets the policy that says what counts as a bug and what counts as done, and the loop executes to that policy until the artifact is clean. The arc is build → polish → ship, and it is one method throughout.
