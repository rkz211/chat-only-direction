...
**The evidence [to be completed at release].** The claim that the loop converges is checkable from the record, and the check is the point of finalizing this appendix later. The `deploy_log` in the loop's persisted state stamps every fix with a timestamp; the defect queue records every report. The convergence curve — *defect-arrival rate over time under a frozen design* — is the single figure that turns this appendix from argument into evidence. `[FIGURE D-1: defect arrivals per day from first frozen-design commit to release, expected monotone-decreasing to zero. TO BE INSERTED AT RELEASE.]` If the curve bends to zero, the finishing phase is a genuine convergence process and the method has a demonstrated full lifecycle. If it plateaus above zero, that is the honest and equally-important finding: it would locate a floor of irreducible LLM-adjudication defects that the corrective guard stack cannot close, and would be the empirical trigger for the deferred architectural move (replacing the LLM adjudicator with a deterministic rules engine for the offending verbs). Either outcome is reported.
